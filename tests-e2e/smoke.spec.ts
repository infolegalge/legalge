...
test("localized smoke", async ({ page, baseURL }) => {
  for (const locale of locales) {
    await test.step(`home (${locale})`, async () => {
      await page.goto(`${baseURL}/${locale}`);
      await expect(page).toHaveTitle(/Legal/);
      await expect(page.getByRole("navigation", { name: /Main/i })).toBeVisible();
    });

    await test.step(`practice index (${locale})`, async () => {
...
test("one dynamic service page renders", async ({ page, baseURL }) => {
  ...
  const candidates = ["migration-to-georgia"];
